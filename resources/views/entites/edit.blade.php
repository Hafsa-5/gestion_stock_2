...
@section('title', 'Modifier une entité')

@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Modifier une entité</h4>
...
@section('content')
    ...
    <div class="section_form">
        <div class="p-5 bg-white">
            <form action="/entites/{{$entite->id}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="">
                    <div class="form-group mb-4">
                        <label class="mb-2" for="nom">Nom : </label>
                        <input name="nom" id="nom" type="text" class="form-control @error('nom') is-invalid @enderror" value="{{ old('nom', $entite->nom) }}">
                        @error('nom')
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="adresse" id="adresse" type="text" class="form-control @error('adresse') is-invalid @enderror" value="{{ old('adresse', $entite->adresse) }}">
                        @error('adresse')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="form-group col-6">
                            ...
                            <input name="phone" id="phone" type="text" class="form-control @error('phone') is-invalid @enderror" value="{{ old('phone', $entite->phone) }}">
                            @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
@section('js')
    <script>
        $(document).ready(function() {
            var selectedVilleId = '{{ old('ville_id', $entite->ville_id) }}';
            var selectedImmeubleId = '{{ old('immeuble_id',$entite->immeuble_id) }}';
