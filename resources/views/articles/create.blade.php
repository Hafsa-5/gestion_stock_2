...
@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Ajouter un nouvau article</h4>
            ...
            <form action="/articles" method="POST" enctype="multipart/form-data">
                ...
                <div class="">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="prix_unitaire" id="prix_unitaire" type="text" class="form-control @error('prix_unitaire') is-invalid @enderror" value="{{ old('prix_unitaire') }}">
                        @error('prix_unitaire')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="quantite_stock" id="quantite_stock" type="text" class="form-control @error('quantite_stock') is-invalid @enderror" value="{{ old('quantite_stock') }}">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="contrat_id" id="contrat_id" class="form-control @error('contrat_id') is-invalid @enderror">
                            ...
                            @foreach($contrats as $contrat)
                                <option value="{{$contrat->id}}" @selected(old('contrat_id') == $contrat->id)>{{$contrat->nom}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="demande_id" id="demande_id" class="form-control @error('demande_id') is-invalid @enderror">
                            ...
                            @foreach($demandes as $demande)
                                <option value="{{$demande->id}}" @selected(old('demande_id') == $demande->id)>{{demande->agent->nom}} {{demande->agent->prenom}} {{$demande->date_demande}}</option>
                            @endforeach
                        </select>
                    </div>
