...
@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Ajouter un nouvau demande</h4>
...
@section('content')
    ...
    <div class="section_form">
        <div class="p-5 bg-white">
            <form action="/demandes" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="py-5">
                    <div class="form-group mb-4">
                        <label class="mb-2" for="date_demande">Date de Demande : </label>
                        <input name="date_demande" id="date_demande" type="date" class="form-control @error('date_demande') is-invalid @enderror" value="{{ old('date_demande') }}">
                        @error('date_demande')
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="agent_id" id="agent_id" class="form-control @error('agent_id') is-invalid @enderror">
                            ...
                            @foreach($agents as $agent)
                                <option value="{{$agent->id}}" @selected(old('agent_id') == $agent->id)>{{$agent->nom}} {{$agent->prenom}}</option>
                            @endforeach
                        </select>
                    </div>
